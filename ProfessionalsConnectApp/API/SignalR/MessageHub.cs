...
public class MessageHub(IUnitOfWork unitOfWork, IMapper mapper,IHubContext<PresenceHub> presenceHub ) : Hub
{
    public override async Task OnConnectedAsync()
    {
        ...
        var messages = await unitOfWork.MessageRespository.GetMessageThread(Context.User.GetUsername(), otherUser!);
    ...
    public async Task SendMessage(CreateMessageDto createMessageDto){
        ...
        var group = await unitOfWork.MessageRespository.GetMessageGroup(groupName);
        ...
        unitOfWork.MessageRespository.AdMessage(message);
    ...
    private async Task<Group> AddToGroup(string groupName){
        ...
        if(group == null){
            ...
            unitOfWork.MessageRespository.AddGroup(group);
        }
    ...
    private async Task<Group> RemoveFromMessageGroup() {
        var group = await unitOfWork.MessageRespository.GetGroupForConnection(Context.ConnectionId);
        var connection = group?.Connections.FirstOrDefault(x => x.ConnectionId == Context.ConnectionId);
        if(connection != null && group != null){
            unitOfWork.MessageRespository.RemoveConnection(connection);
            if(await unitOfWork.Complete()) return group;
        }
